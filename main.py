...
def get_realization(s: list[note.Note]):
    '''
...
def realization_similiarity(r1: tuple, r2: tuple):
    '''
    Computes similiarity coefficient of two realizations (should be the same length)
    '''
    pass

def get_motive(s: stream.Stream, k: int):
    '''
...
def composition_similiarity(c1: list[list[tuple]], c2: list[list[tuple]]):
    ...
    Each motive has only one best fitted motive from the other composition
    '''
    pass
